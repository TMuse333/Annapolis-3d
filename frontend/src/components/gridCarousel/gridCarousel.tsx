...
const CarouselGrid: React.FC<CarouselGridProps> = ({ images, isMobile, bgColor }) => {
  ...
  function handleCarouselClick(index: number | null) {
    if (index !== null) {
      ...
      setShift(-index);
      // Reset edge case states to ensure consistent initialization
      setLeftEdgeCase(index === 0);
      setLeftEdgeShift(index === images.length - 1 ? 0 : -100);
      setRightEdgeShift(index === 0 ? 100 : 0);
    } else {
  ...
  useEffect(() => {
    if (shift === 0) {
      ...
      setLeftEdgeShift((shift + images.length - 1) * 100);
    ...
    console.log('shift', shift, 'currentImage', currentImage);
  }, [shift, currentImage, leftClicked, rightClicked, leftEdgeCase, images.length]);
  ...
  return (
    <section className={`w-full ${bgColor ? bgColor : ''}`}>
                ...
                <div
                  ...
                  style={{
                    transform: `translateX(${
                      index === images.length - 1 ? leftEdgeShift : index === 0 ? rightEdgeShift : shift * 100 + 100 * index
                    }%)`,
                  }}
                >
                  <Image
                      ...
                      ${
                        !carouselClicked
                          ? `w-[100%] max-w-[805px] max-h-[624px] md:max-h-[750px] md:max-w-[1200px] ${
                              !image.objectCover ? 'object-contain' : 'object-cover'
                            } z-[29]`
                          : `w-[100vw] max-w-[1400px] ${!image.objectCover ? 'object-contain' : 'object-cover'} ml-auto mr-auto h-[100vh]`
                      }
        ...
        <section className="w-screen max-w-[1800px] mx-auto grid grid-cols-2 md:grid-cols-3 justify-center">
          {images.map((image, index) => (
            <Image
              ...
              alt={image.alt}
              key={index}
              className="w-[40vw] md:w-[31vw] object-contain mx-auto max-w-[480px] max-h-[320px] mb-8 hover:scale-[1.1] transition-transform"
              onClick={() => handleCarouselClick(index)}
              width={600}
              height={1300}
            />
