...
const Carousel = ({ images, hasDescription, button }: CarouselProps): React.JSX.Element => {
  ...
  return (
    <section
      ...
      className={`w-screen text-black flex flex-col ml-auto mr-auto justify-center items-center md:flex-row rounded-2xl
      bg-gradient-to-br from-[#1E90FF]  to-[#4EA4FF]
    ...
    >
        ...
        <div
          className={`flex relative justify-center items-center ml-auto mr-auto 
            ${!carouselClicked || hasDescription ? `
              w-[90vw] max-h-[804px] h-[95vw] rounded-2xl
              max-w-[900px] max-h-[420px]
        ...
        >
          {images.map((image, index) => (
            <div
              ...
              className={`z-[33] ml-auto mr-auto absolute top-0 rounded-2xl px-4 flex flex-col
                ${!carouselClicked || hasDescription ? `
                  w-[100vw] h-[80vw] max-h-[400px] 
            ...
            >
              {hasDescription && (
                <AnimatePresence mode="wait">
                  <motion.h4
                    ...
                    className="md:hidden text-center text-2xl mb-3"
                  >
              ...
              {image.isVideo ? (
                <video
                  className={`
                    ${!carouselClicked || hasDescription ? `w-[80%] max-w-[805px] max-h-[624px] md:max-w-[700px] h-[100%]` 
                      : `w-[100vw] max-w-[1400px] ml-auto mr-auto max-h-[900px]`}
                    object-contain z-[65] ml-auto mr-auto`}
                  width={600}
                  height={1300}
                  autoPlay
                  loop
                ...
                <Image
                  ...
                  height={1300}
                  className={`
                    ${!carouselClicked || hasDescription ? `w-[100%] max-w-[805px] max-h-[624px] md:max-w-[700px] h-[100%]` 
                      : `w-[100vw] max-w-[1400px] ml-auto mr-auto max-h-[900px]`}
                    ${image.objectContain ? 'object-contain' : 'object-cover'} z-[25] ml-auto mr-auto`}
                />
              )}
            </div>
          ))}
          ...
          <div className={`${!carouselClicked || hasDescription ? `
            max-w-[805px] max-h-[624px] md:max-w-[700px] w-[100%] absolute top-0 h-[100%] max-w-[900px]` 
            : 'w-screen max-w-[1575px] h-screen relative'} z-[45]`}
          >
      ...
      {hasDescription && (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.7 }}
          >
            <h4 className="hidden md:block text-center text-5xl font-semibold">
              {images[currentImage].title}
            </h4>
            <p className="text-center md:text-left md:mt-[3rem] px-3 md:pl-8 text-lg sm:text-xl">
              {images[currentImage].description}
              {/* {images[currentImage].link && button && (
                <Link href={images[currentImage].link? images[currentImage].link : "/home"}>
                  <button className="p-3 rounded-2xl mt-4 transition-transform hover:scale-[1.05]">
                    {button.buttonText}
                  </button>
                </Link>
              )} */}
      ...
      {carouselClicked && !hasDescription && (
        <button
          className="fixed bottom-[2%] left-[50%] -translate-x-[50%] z-[100] p-2 rounded-xl"
          onClick={handleCarouselClick}
        >
          Collapse
        </button>
