...
const ExperienceCard = ({
  ...
  return (
    <motion.section
      ref={ref}
      style={{ scale }}
      className="relative mx-auto w-[98vw] rounded-2xl my-8 max-w-[1200px] overflow-x-hidden
      bg-gradient-to-t from-[#87CEEB] to-[#1E90FF]"
    >
      <h2
        ref={headerRef}
        id={`${title}-header`}
      ...
      <section
        className={`flex flex-col md:px-4 mx-auto ${
      ...
      >
        ...
        >
          ...
          <br />
          {link !== "" && (
            <button className="p-2 rounded-2xl mt-3 transition-all hover:scale-[1.1]">
      ...
      <section className="w-full">
        <h3 className="text-center my-6 text-3xl font-semibold px-4">
          {aspectHeader}
        </h3>
        <ul
          ref={liRef}
        ...
        >
          {aspects.map((aspect, index) => (
            <motion.li
              className="mb-4 w-[90%] max-w-[250px] mx-auto p-3 rounded-2xl flex justify-center items-center text-center md:w-full md:mr-4
              bg-blue-300"
              key={index}
              variants={liVariants(
                isMobile ? index * 0.2 : index * 0.1,
                index
              )}
              initial="initial"
